...
//   node scripts/precompute-postflop.mjs --no-index         # skip writing the shared index
//
// Parallel mode (many cores): run scripts/precompute-postflop-parallel.mjs
// instead — it fans boards out across N worker processes (default 8 boards
// at once) and writes the shared index exactly once at the end.
//
...
// --no-index: skip writing the shared postflop-solution-index file at the end.
// Used by scripts/precompute-postflop-parallel.mjs so concurrent workers never
// race on the index; the parallel driver writes it once after all workers finish.
